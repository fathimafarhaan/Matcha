...
const AdminDashboard = () => {
  ...
  const fetchScoringCriteria = useCallback(async () => {
    ...
    try {
      const response = await axios.get("http://localhost:5000/admin/get_scoring_criteria", {
  ...
  const handleUpdateCriteria = async (e) => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/admin/update_scoring",
        {
  ...
  const handleDownloadCSV = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:5000/admin/download_csv", {
        headers: { Authorization: `Bearer ${token}` },
        responseType: "blob",
      });
